(function () {
  ...
  function monthDay(day) {
    ...
    return months[Number(parts[1]) - 1] + ' ' + String(Number(parts[2])).padStart(2, '0');
  }
  ...
  function renderDay(svg, payload) {
    ...
    var group = svg.closest ? svg.closest('.lineage-day-group') : null;
    var note = group && group.querySelector ? group.querySelector('.lineage-js-note') : null;
    ...
    nodes.forEach(function (node) {
      ...
      if (node.parent_day) {
        ...
        stub.setAttribute('y', position.y - 14);
        stub.setAttribute('text-anchor', 'middle');
        stub.textContent = '↩ from ' + monthDay(node.parent_day);
        svg.appendChild(stub);
      }
      if (node.current || (payload.current_sha && node.sha === payload.current_sha)) {
        var star = svgEl('text');
  ...
  function selectDays(mode, days, nowValue) {
    ...
    if (mode !== 'range' && days.indexOf(today) === -1) {
      note = 'No data for ' + today + ' yet' + (latest ? '; latest day with data: ' + latest : '') + '.';
    }
