...
def build_two_tier_context_html(agent_context: dict[str, Any] | None) -> str:
    ...
    out.append(f'        <div class="t1-block-item t1-msg"><span class="t1-seq">msg</span><span class="t1-name">history + tool results</span><span class="t1-desc">{history_text}</span></div>')
    ...
    missing = [str(name) for name in memory_index.get("resident_missing", [])] if isinstance(memory_index, dict) else []
    out.append(f'<div class="context-telemetry-box"><strong>Memory index:</strong> status {esc(memory_index.get("status", "unavailable") if isinstance(memory_index, dict) else "unavailable")} · resident matched {memory_index.get("resident_matched", "unavailable") if isinstance(memory_index, dict) else "unavailable"} · resident missing: {esc(", ".join(missing) if missing else "none")}</div>')
    window_tokens = 98000
    reserve_tokens = 8000
    ...
    reserve_tokens = 8000
    window_budget = window_tokens - reserve_tokens
    compaction_status = "compaction did not fire" if isinstance(compaction, dict) and compaction.get("status") == "empty" else esc(compaction.get("status", "unavailable") if isinstance(compaction, dict) else "unavailable")
    out.append(f'<div class="context-telemetry-box"><strong>Dialogue Window:</strong> {window_budget:,} tokens ({window_tokens:,} − {reserve_tokens:,}) · occupancy: {history_text} · {compaction_status}</div>')
    ...
    for name in sorted(name for name in skill_rows if name):
        ...
        state = "unreachable" if record.get("omitted") and record.get("on_disk") else "working" if record.get("on_disk") else "not on disk"
        skill_table_rows.append(f'<tr><td>{esc(name)}</td><td>{"yes" if record.get("omitted") is not True else "no"}</td><td>{"yes" if record.get("on_disk") else "no"}</td><td>{record.get("reads", 0)}</td><td>{record.get("confirmed", 0)}</td><td>{state}</td></tr>')
    out.append(f'<div class="context-telemetry-box"><strong>Skill source join:</strong></div><table class="skills-table"><thead><tr><th>Skill</th><th>In catalogue</th><th>On disk</th><th>Reads</th><th>Confirmed</th><th>State</th></tr></thead><tbody>{"".join(skill_table_rows)}</tbody></table>')
